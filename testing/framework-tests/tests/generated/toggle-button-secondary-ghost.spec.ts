...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('toggle-button-secondary-ghost - accessibility check', async ({ page }) => {
    await page.goto('/preview/toggle-button-secondary-ghost');
...
test('toggle-button-secondary-ghost', async ({ page }) => {
  await page.goto('/preview/toggle-button-secondary-ghost');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'toggle-button-secondary-ghost.aria-snapshot.yaml',
  });
});
