...
test('generic-list-notification-icon', async ({ page }) => {
  await page.goto('/preview/generic-list-notification-icon');

  // Ugly and not the reliable way to wait for Stencil to be ready
  await waitForReadiness(page);

  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'generic-list-notification-icon.aria-snapshot.yaml',
  });
});
