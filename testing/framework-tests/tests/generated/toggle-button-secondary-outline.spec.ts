...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('toggle-button-secondary-outline - accessibility check', async ({ page }) => {
    await page.goto('/preview/toggle-button-secondary-outline');
...
test('toggle-button-secondary-outline', async ({ page }) => {
  await page.goto('/preview/toggle-button-secondary-outline');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'toggle-button-secondary-outline.aria-snapshot.yaml',
  });
});
