...
test('generic-list-additional-options', async ({ page }) => {
  await page.goto('/preview/generic-list-additional-options');

  // Ugly and not the reliable way to wait for Stencil to be ready
  await waitForReadiness(page);

  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'generic-list-additional-options.aria-snapshot.yaml',
  });
});
