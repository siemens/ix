...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('button-grey-secondary - accessibility check', async ({ page }) => {
    await page.goto('/preview/button-grey-secondary');
...
test('button-grey-secondary', async ({ page }) => {
  await page.goto('/preview/button-grey-secondary');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'button-grey-secondary.aria-snapshot.yaml',
  });
});
