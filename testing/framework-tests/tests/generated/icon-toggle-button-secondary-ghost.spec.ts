...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('icon-toggle-button-secondary-ghost - accessibility check', async ({ page }) => {
    await page.goto('/preview/icon-toggle-button-secondary-ghost');
...
test('icon-toggle-button-secondary-ghost', async ({ page }) => {
  await page.goto('/preview/icon-toggle-button-secondary-ghost');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'icon-toggle-button-secondary-ghost.aria-snapshot.yaml',
  });
});
