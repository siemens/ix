...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('button-danger-ghost - accessibility check', async ({ page }) => {
    await page.goto('/preview/button-danger-ghost');
...
test('button-danger-ghost', async ({ page }) => {
  await page.goto('/preview/button-danger-ghost');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'button-danger-ghost.aria-snapshot.yaml',
  });
});
