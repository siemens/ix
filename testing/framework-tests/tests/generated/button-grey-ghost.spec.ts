...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('button-grey-ghost - accessibility check', async ({ page }) => {
    await page.goto('/preview/button-grey-ghost');
...
test('button-grey-ghost', async ({ page }) => {
  await page.goto('/preview/button-grey-ghost');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'button-grey-ghost.aria-snapshot.yaml',
  });
});
