...
if (process.env.DO_ACCESSIBILITY_AUDIT) {
  test('icon-toggle-button-primary-ghost - accessibility check', async ({ page }) => {
    await page.goto('/preview/icon-toggle-button-primary-ghost');
...
test('icon-toggle-button-primary-ghost', async ({ page }) => {
  await page.goto('/preview/icon-toggle-button-primary-ghost');
  ...
  await expect(page.locator('body')).toMatchAriaSnapshot({
    name: 'icon-toggle-button-primary-ghost.aria-snapshot.yaml',
  });
});
