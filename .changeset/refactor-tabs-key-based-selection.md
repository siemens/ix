...
- Removed `ariaLabelChevronLeftIconButton` and `ariaLabelChevronRightIconButton` props. When the tab list overflows, `ix-tabs` now shows an `ix-dropdown-button` with all tabs instead of scroll arrows; no overflow button is rendered while all tabs fit.
