...
function tryToRemoveTagImports(file: string) {
  const regex = /import (.*) from '.*(ix-.*)\/tags.md';/g;

  return file.replace(regex, '');
}

function getAllTagImports(name: string, file: string) {
  const indexImports: GenericImports[] = [];
  const regex = /import (.*) from '.*(ix-.*)\/tags.md';/g;
  const matches = Array.from(file.matchAll(regex));
  matches.forEach((match) => {
    const [, importName, componentTag] = matches[0];
    indexImports.push({
      importName,
      componentTag,
    });
...
Object.keys(newDocs).forEach((name) => {
  ...
  const indexTagImports: GenericImports[] = [];
  ...
  if (codePath) {
    ...
    indexTagImports.push(...getAllTagImports(name, codeFile));
    codeFile = tryToRemoveTagImports(codeFile);
