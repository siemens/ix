...
const __previewMobile = path.join(__preview, 'mobile');
...
async function generateApiMarkdown() {
  ...
  for (const component of components) {
    ...
    await fs.writeFile(
      path.join(__component, `props.mdx`),
      escapeMarkdown(propertyOutput)
    );
    await fs.writeFile(
      path.join(__component, `events.mdx`),
      escapeMarkdown(eventOutput)
    );
    await fs.writeFile(
      path.join(__component, `slots.mdx`),
      escapeMarkdown(slotOutput)
    );
    await fs.writeFile(
      path.join(__component, `api.mdx`),
      escapeMarkdown(apiOutput)
    );
    await fs.writeFile(
      path.join(__component, `tags.mdx`),
      escapeMarkdown(tagsOutput)
    );
  }
}
