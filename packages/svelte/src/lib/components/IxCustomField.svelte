...
<script lang="ts">
  ...
  let {
    ...
    element = $bindable(),
    ...rest
  }: StencilSvelteProps<JSX.IxCustomField, HTMLIxCustomFieldElement> = $props();

  const stencil = useStencilElement(() => element, () => rest, PROPS, EVENTS);
</script>

<ix-custom-field bind:this={element} {...stencil.attributes}>{@render children?.()}</ix-custom-field>
