...
<script lang="ts">
  ...
  let {
    ...
    element = $bindable(),
    ...rest
  }: StencilSvelteProps<JSX.IxPopoverContent, HTMLIxPopoverContentElement> = $props();

  const stencil = useStencilElement(() => element, () => rest, PROPS, EVENTS);
</script>

<ix-popover-content bind:this={element} {...stencil.attributes}>{@render children?.()}</ix-popover-content>
