...
<script lang="ts">
  ...
  let {
    ...
    element = $bindable(),
    ...rest
  }: StencilSvelteProps<JSX.IxDropdownItem, HTMLIxDropdownItemElement> = $props();

  const stencil = useStencilElement(() => element, () => rest, PROPS, EVENTS);
</script>

<ix-dropdown-item bind:this={element} {...stencil.attributes}>{@render children?.()}</ix-dropdown-item>
