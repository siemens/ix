...
<script lang="ts">
  ...
  let {
    ...
    element = $bindable(),
    ...rest
  }: StencilSvelteProps<JSX.IxMenuAboutItem, HTMLIxMenuAboutItemElement> = $props();

  const stencil = useStencilElement(() => element, () => rest, PROPS, EVENTS);
</script>

<ix-menu-about-item bind:this={element} {...stencil.attributes}>{@render children?.()}</ix-menu-about-item>
