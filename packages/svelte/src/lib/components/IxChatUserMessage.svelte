...
<script lang="ts">
  ...
  let {
    ...
    element = $bindable(),
    ...rest
  }: StencilSvelteProps<JSX.IxChatUserMessage, HTMLIxChatUserMessageElement> = $props();

  const stencil = useStencilElement(() => element, () => rest, PROPS, EVENTS);
</script>

<ix-chat-user-message bind:this={element} {...stencil.attributes}>{@render children?.()}</ix-chat-user-message>
