...
const obsoleteDesignTokenPattern =
  /--si-sys-(?:background|border|text|effects|data|code)-|\$si-sys-(?:background|border|text|effects|data|code)-/g;
const directSassSizingTokenPattern =
  /(?:(?:[a-zA-Z_][a-zA-Z0-9_-]*)\s*\.\s*)?\$si-sys-sizing-[a-zA-Z0-9-]+/g;
const generatedFileMarker = 'NOTE: This file is automatically generated.';
const stencilGeneratedFileMarker =
...
describe('component CSS custom properties', () => {
  it('does not consume obsolete design token names', () => {
    const violations = repositorySourceFiles.flatMap((sourcePath) => {
      const source = fs.readFileSync(sourcePath, 'utf8');

      if (
        source.includes(generatedFileMarker) ||
        source.includes(stencilGeneratedFileMarker)
      ) {
        return [];
      }

      return [...source.matchAll(obsoleteDesignTokenPattern)].map(
        (match) => `${path.relative(repositoryRoot, sourcePath)}: ${match[0]}`
      );
    });
  ...
  it.each([
    '$si-sys-sizing-size-80',
    'sys.$si-sys-sizing-icon-lg',
    'density_tokens . $si-sys-sizing-spacing-x-40',
    '#{alternate.$si-sys-sizing-border-radius-sm}',
    `calc(
      #{tokens
        . $si-sys-sizing-size-100}
      + #{tokens.$si-sys-sizing-spacing-y-20}
    )`,
  ])('detects direct Sass sizing references in %s', (source) => {
    expect([...source.matchAll(directSassSizingTokenPattern)]).not.toEqual([]);
  });

  it('keeps component sizing system tokens in variable files', () => {
    const violations = findFiles(componentsRoot, '.scss')
      .filter((sourcePath) => !sourcePath.endsWith('.vars.scss'))
      .flatMap((sourcePath) => {
        const source = fs.readFileSync(sourcePath, 'utf8');
        ...
        return [...source.matchAll(directSassSizingTokenPattern)].map(
          (match) => `${path.relative(componentsRoot, sourcePath)}: ${match[0]}`
        );
      });
