...
regressionTest(
  'modal appears at top when centered is false',
  async ({ mount, page }) => {
    await mount('');
    await setupModalEnvironment(page);
    ...
    const dialog = page.locator('ix-modal dialog');
    await expect(dialog).toBeVisible();
    const top = await dialog.evaluate((el) => getComputedStyle(el).top);
    expect(parseInt(top)).toBeLessThanOrEqual(10);
  }
);

regressionTest(
  'modal appears centered when centered is true',
  async ({ mount, page }) => {
    await mount('');
    await setupModalEnvironment(page);
    ...
    await expect(dialog).toBeVisible();
    const top = await dialog.evaluate((el) => getComputedStyle(el).top);
    expect(parseInt(top)).toBeGreaterThan(10);
  }
);
