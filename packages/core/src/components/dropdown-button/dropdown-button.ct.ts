...
import { iconCheckboxes, iconRocket } from '@siemens/ix-icons/icons';
...
regressionTest(
  'uses the computed trigger label as the menu accessible name',
  async ({ mount, page }) => {
    await mount(`
    <ix-dropdown-button
      aria-label-dropdown-button="Available actions"
      label="Actions"
    >
      <ix-dropdown-item label="Edit"></ix-dropdown-item>
    </ix-dropdown-button>
  `);
    ...
    await expect(
      page.getByRole('menu', { name: 'Available actions' })
    ).toBeVisible();
  }
);
