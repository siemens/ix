...
regressionTest('required with novalidate=false (default)',
  async ({ mount, page }) => {
    await mount(
      `
      <form>
        <ix-radio-group label="Required Group" required>
          <ix-radio label="Option 1" value="option1" required></ix-radio>
          <ix-radio label="Option 2" value="option2" required></ix-radio>
        </ix-radio-group>
    ...
    const group = page.locator('ix-radio-group');
    ...
    await expect(group).not.toHaveClass(/ix-invalid/);
    await submitButton.click();
    await expect(group).toHaveClass(/ix-invalid--required/);
...
regressionTest('required with novalidate=true disables validation',
  async ({ mount, page }) => {
    ...
    await submitButton.click();
    await expect(group).not.toHaveClass(/ix-invalid--required/);
