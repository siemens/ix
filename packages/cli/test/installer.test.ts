...
import { IxPatternsConfig, loadConfig, saveConfig } from '../src/config';
import {
  applyInstallPlan,
  preparePatternInstall,
  reportInstallPlan,
} from '../src/installer';
import { PatternDefinition } from '../src/registry';

const packageRoot = fileURLToPath(new URL('..', import.meta.url));
const workRoot = path.join(packageRoot, '.test-work', 'installer');
const baseUrl = 'https://registry.example/root';
const patternEntryPath = 'v1/patterns/card.json';
...
async function prepare(
  ...
  const definition = options.definition ?? patternDefinition();
  return preparePatternInstall({
    cwd,
    baseUrl,
    patternEntryPath,
    patternDef: definition,
    expectedPatternName: 'card',
    framework: 'react',
    tokens: {},
    targetFolder: config.targetFolder,
    previousFiles: config.patterns.find((pattern) => pattern.name === 'card')
      ?.files,
    force: options.force,
    fetchImpl:
      options.fetchImpl ??
      fetchFiles({
        [`${baseUrl}/v1/patterns/react/card.tsx`]: content,
      }),
  });
}
...
test('performs a clean install and clean tracked update', async () => {
  ...
  const output = path.join(cwd, 'src/features/patterns/card/card.tsx');
  assert.equal(await fs.readFile(output, 'utf8'), 'first');
  assert.equal(firstConfig.patterns[0].files?.length, 1);
  ...
  assert.equal((await loadConfig(cwd)).patterns[0].version, 'v2');
});

test('rejects pattern files that are not prefixed with the selected framework', async () => {
  const { cwd, config } = await createProject('wrong-framework-path');

  await assert.rejects(
    prepare(cwd, config, 'content', {
      definition: patternDefinition([{ path: 'angular/card.tsx' }]),
    }),
...
test('rejects a customized tracked file and preserves it', async () => {
  ...
  assert.deepEqual(plan.conflicts, [
    {
      path: 'src/features/patterns/card/card.tsx',
      type: 'modified',
    },
  ]);
  await assert.rejects(
  ...
  assert.equal((await loadConfig(cwd)).patterns[0].version, 'v1');
});
...
test('leaves zero project writes when any fetch fails', async () => {
  ...
  const lockBefore = await fs.readFile(
    path.join(cwd, 'ix-patterns-lock.json'),
    'utf8'
  );
  ...
  await assert.rejects(
    prepare(cwd, config, '', {
      ...
      fetchImpl: fetchFiles({
        [`${baseUrl}/v1/patterns/react/card.tsx`]: 'ok',
        [`${baseUrl}/v1/patterns/react/card.css`]: 503,
      }),
    }),
    /503/
  );
  await assert.rejects(
    fs.access(path.join(cwd, 'src/features/patterns/card/card.tsx'))
  );
  assert.equal(
    await fs.readFile(path.join(cwd, 'ix-patterns-lock.json'), 'utf8'),
    lockBefore
  );
});
...
test('rolls back earlier writes when applying a later file fails', async () => {
  ...
  const plan = await prepare(cwd, config, '', {
    definition,
    fetchImpl: fetchFiles({
      [`${baseUrl}/v1/patterns/react/card.tsx`]: 'component',
      [`${baseUrl}/v1/patterns/react/card.css`]: 'styles',
    }),
  });
  ...
  await assert.rejects(
    fs.access(path.join(cwd, 'src/features/patterns/card/card.css'))
  );
  assert.equal(
    await fs.readFile(path.join(cwd, 'ix-patterns-lock.json'), 'utf8'),
    lockBefore
...
test('does not overwrite a file changed after planning', async () => {
  ...
  assert.deepEqual((await loadConfig(cwd)).patterns, []);
});
...
test('rejects redirected pattern payload responses', async () => {
