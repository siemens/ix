...
2. Start the playwright docker container (Currently *v1.45.1-noble*, but can be a newer version) mounting the current working directory into the containers *work* directory:

```sh
docker run -v $(pwd):/work/ -w /work -it mcr.microsoft.com/playwright:v1.45.1-noble /bin/bash
```
