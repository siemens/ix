...
function parseArgs(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (name === "help") {
			console.log("Search versioned Siemens iX registry documentation. Use --local-index for offline searches. Without --version, the installed version in --project-dir is preferred.");
			process.exit(0);
		}
		const value = argv[index + 1];
		if (!value || value.startsWith("--")) usageError(`Missing value for --${name}`);
		index += 1;
		switch (name) {
			case "query":
			...
			case "limit":
				...
				if (!Number.isInteger(options.limit) || options.limit < 0) usageError("--limit must be a non-negative integer");
				break;
			case "local-index":
			case "index":
			...
			case "component-name":
				...
				break;
			default: usageError(`Unknown option '--${name}'`);
		}
	}
	if (options.queries.length === 0 && options.figmaIds.length === 0 && options.componentNames.length === 0) usageError("at least one of --query, --figma-id, or --component-name is required");
	if (options.kind && !COMPONENT_KINDS.includes(options.kind)) usageError(`Invalid kind '${options.kind}'`);
	if (options.framework && !FRAMEWORKS.includes(options.framework)) usageError(`Invalid framework '${options.framework}'`);
	return options;
}
function normalizeFigmaId(value) {
	const normalized = String(value).trim();
	return /^\d+[-:]\d+$/.test(normalized) ? normalized.replace("-", ":") : normalized;
}
function normalizeName(value) {
	return String(value).trim().toLowerCase().replace(/^ix[-_]/, "").replace(/^ix/, "").replace(/[^a-z0-9]/g, "");
}
function tagToReactName(tag) {
	return `Ix${tag.replace(/^ix-/, "").split("-").map((part) => `${part.slice(0, 1).toUpperCase()}${part.slice(1)}`).join("")}`;
}
...
function normalizePath(value, pathPrefix, knownVersions) {
	...
	if (knownVersions.includes(head) && rest.length > 0) return `${pathPrefix}/${rest.join("/")}`;
...
function exactMatchPriority(result, query) {
	...
	if (names.some((value) => value.replace(/^ix-/, "") === rawQuery.replace(/^ix-/, ""))) return 3;
	if (result.figmaMainComponentIds?.some((id) => normalizeFigmaId(id.toLowerCase()) === normalizedQuery)) return 2;
	return 0;
}
function parseEnvelope(value, source) {
	if (!value || typeof value !== "object" || value.schemaVersion !== 1 || !Array.isArray(value.fields) || value.fields.length === 0 || !value.fields.every((field) => typeof field === "string" && field) || !Array.isArray(value.storeFields) || value.storeFields.length === 0 || !value.storeFields.every((field) => typeof field === "string" && field) || !value.searchOptions || typeof value.searchOptions !== "object" || Array.isArray(value.searchOptions) || !value.searchOptions.boost || typeof value.searchOptions.boost !== "object" || Array.isArray(value.searchOptions.boost) || typeof value.searchOptions.fuzzy !== "number" || !Number.isFinite(value.searchOptions.fuzzy) || typeof value.searchOptions.prefix !== "boolean" || !value.payload || typeof value.payload !== "object" || Array.isArray(value.payload) || ![
		"documentCount",
		"documentIds",
		"fieldIds",
		"fieldLength",
	...
	].every((field) => Object.prototype.hasOwnProperty.call(value.payload, field))) throw new SearchError(`Invalid documentation search index '${source}': expected schemaVersion 1 and a self-describing MiniSearch payload`, "error");
	for (const [field, boost] of Object.entries(value.searchOptions.boost)) if (typeof boost !== "number" || !Number.isFinite(boost) || boost <= 0) throw new SearchError(`Invalid documentation search index '${source}': boost for '${field}' must be a positive number`, "error");
	return value;
}
...
async function packageRootFrom(projectDir, packageName) {
	...
	for (const root of roots) {
		...
		try {
			...
			if (!metadata.name || metadata.name === packageName) return {
				root,
				metadata
			};
		} catch {}
...
async function fetchJson(url, description) {
	...
	try {
		...
		throw new SearchError(`Failed to fetch ${description}: ${error instanceof Error ? error.message : String(error)}`, "version_unavailable");
	}
	if (!response.ok) throw new SearchError(`Failed to fetch ${description}: ${response.status}`, "version_unavailable");
	try {
		return await response.json();
	} catch (error) {
		throw new SearchError(`Failed to parse ${description}: ${error instanceof Error ? error.message : String(error)}`, "version_unavailable");
	}
}
function resolveRegistryVersion(registry, requestedVersion) {
	const versions = registry?.versions ?? {};
	const candidate = (requestedVersion && registry["dist-tags"]?.[requestedVersion]) ?? requestedVersion ?? registry["dist-tags"]?.latest;
	if (!candidate) return null;
	if (Object.hasOwn(versions, candidate)) return candidate;
	if (candidate.startsWith("v") && Object.hasOwn(versions, candidate.slice(1))) return candidate.slice(1);
	if (Object.hasOwn(versions, `v${candidate}`)) return `v${candidate}`;
	return null;
}
async function loadRegistryIndex(options, installedVersion) {
	if (!options.version && !installedVersion) throw new SearchError("Could not resolve an installed IX version. Provide --version or install @siemens/ix (or a compatible framework wrapper) in --project-dir.", "version_unavailable");
	...
	if (!version) throw new SearchError(`Unknown registry version or tag '${requestedVersion ?? "<latest>"}'`, "version_unavailable", { version: requestedVersion ?? null });
	const indexPath = registry.versions[version]?.documentationSearchIndex;
	if (typeof indexPath !== "string") throw new SearchError(`Registry version '${version}' does not define documentationSearchIndex`, "version_unavailable", { version });
	return {
		envelope: parseEnvelope(await fetchJson(`${registryUrl}/${indexPath}`, "documentation search index"), indexPath),
...
function storedDocuments(envelope) {
	return Object.values(envelope.payload.storedFields ?? {}).filter((document) => document && typeof document === "object");
}
function stringList(values) {
	return [...new Set(values.flatMap((value) => Array.isArray(value) ? value : [value]).filter((value) => typeof value === "string").map((value) => value.trim()).filter(Boolean))].sort((left, right) => left.localeCompare(right));
}
function docsTagValues(component, tagName) {
	return stringList((component.docsTags ?? []).filter((tag) => tag?.name === tagName).flatMap((tag) => String(tag.text ?? "").split(",")));
}
...
function relativeProjectPath(projectDir, filePath) {
	return path.relative(projectDir, filePath).replaceAll(path.sep, "/") || filePath;
}
...
async function readReactSymbols(reactPackage) {
	if (!reactPackage) return /* @__PURE__ */ new Set();
	const distributionDirectory = path.join(reactPackage.root, "dist");
	const files = [...await collectDeclarationFiles(path.join(distributionDirectory, "types")), ...await collectFiles(distributionDirectory, ".js")];
	const symbols = /* @__PURE__ */ new Set();
	for (const file of files) {
		const source = await fs.readFile(file, "utf8");
		for (const match of source.matchAll(/\bexport\s+(?:declare\s+)?(?:const|class|function)\s+(Ix[A-Z][A-Za-z0-9]*)\b/g)) if (match[1]) symbols.add(match[1]);
		for (const match of source.matchAll(/\bexport\s*{([^}]*)}/g)) for (const specifier of (match[1] ?? "").split(",")) {
			const exportedName = specifier.trim().split(/\s+as\s+/).at(-1);
			if (/^Ix[A-Z][A-Za-z0-9]*$/.test(exportedName ?? "")) symbols.add(exportedName);
		}
	}
	return symbols;
}
function interfaceBodies(source) {
	...
	for (const match of source.matchAll(/interface\s+(Ix[A-Z][A-Za-z0-9]*)\s*{/g)) {
		const name = match[1];
...
function componentTagFromReactName(name) {
	return `ix-${name.replace(/^Ix/, "").replace(/([a-z0-9])([A-Z])/g, "$1-$2").replace(/([A-Z])([A-Z][a-z])/g, "$1-$2").toLowerCase()}`;
}
async function declarationComponents(corePackage, projectDir, reactSymbols) {
	...
	const files = [...await collectDeclarationFiles(path.join(corePackage.root, "dist", "types"))];
	...
	for (const declarationPath of [
		...
		if (file.endsWith(".d.ts") && await pathExists(file)) files.push(file);
	}
	const uniqueFiles = [...new Set(files)].sort();
	const texts = await Promise.all(uniqueFiles.map(async (file) => ({
		file,
	...
	for (const { file, source } of texts) {
		for (const match of source.matchAll(/\b(ix-[a-z0-9-]+)\b/g)) tags.add(match[1]);
		const fileName = path.basename(file);
		if (/^ix-[a-z0-9-]+\.d\.ts$/.test(fileName)) tags.add(fileName.slice(0, -5));
	}
	for (const { source } of texts) for (const [name, body] of interfaceBodies(source)) {
	...
	return [...tags].sort((left, right) => left.localeCompare(right)).map((tag) => {
		...
		const apiText = stringList([
			bodies.get(tag) ?? "",
			...texts.filter(({ source: text }) => text.includes(`"${tag}"`)).map(({ source }) => source),
			...sourceFile ? texts.filter(({ file }) => file === sourceFile).map(({ source }) => source) : []
		]);
		return {
			id: `component:${tag}`,
			kind: "component",
			name: tag,
			tag,
			aliases: stringList([tag, ...reactSymbols.has(reactName) ? [reactName] : []]),
			description: "Declaration-only metadata; component summary unavailable.",
			keywords: stringList([
				tag,
				...reactSymbols.has(reactName) ? [reactName] : [],
				apiText
			]).join(" "),
			path: relativeProjectPath(projectDir, sourceFile ?? path.join(corePackage.root, "dist", "types", "components.d.ts")),
			detailPath: relativeProjectPath(projectDir, sourceFile ?? path.join(corePackage.root, "dist", "types", "components.d.ts")),
			relatedComponents: [],
			relatedExamples: [],
			reactExamples: [],
			relatedBlocks: [],
			...
			files: uniqueFiles.filter((file) => file === sourceFile).map((file) => relativeProjectPath(projectDir, file)).join(" "),
			sourceText: `${tag} ${reactSymbols.has(reactName) ? reactName : ""} ${apiText.join(" ")}`
		};
	});
}
async function componentDocDocuments(corePackage, projectDir, reactSymbols) {
	...
	const componentDocRelativePath = relativeProjectPath(projectDir, componentDocPath);
	return (await Promise.all(componentDoc.components.map(async (component) => {
		const tag = component.tag;
		const apiMembers = componentApiText(component);
		const documentation = docsTagValues(component, "documentation");
		const figmaMainComponentIds = docsTagValues(component, "figma-main-component-id").map(normalizeFigmaId);
		const relatedComponents = stringList([...component.dependencies ?? [], ...component.dependents ?? []]);
		const aliases = stringList([tag, ...reactSymbols.has(tagToReactName(tag)) ? [tagToReactName(tag)] : []]);
		const apiDocsPath = path.join(corePackage.root, "api-docs", "components", tag.replace(/^ix-/, ""), "readme.md");
		const detailPath = await pathExists(apiDocsPath) ? relativeProjectPath(projectDir, apiDocsPath) : componentDocRelativePath;
		return {
			id: `component:${tag}`,
			kind: "component",
			name: tag,
...
async function loadInstalledFallback(projectDir, packages) {
	...
	const reactSymbols = await readReactSymbols(packages.wrappers.find(({ packageName }) => packageName === "@siemens/ix-react"));
	const metadataDocuments = await componentDocDocuments(packages.core, projectDir, reactSymbols);
	if (metadataDocuments) return {
		...createMemoryIndex(metadataDocuments),
		source: "installed-component-doc",
		version: packages.core.metadata.version ?? null,
		authoritativeFigmaMappings: true
	};
	const declarationDocuments = await declarationComponents(packages.core, projectDir, reactSymbols);
...
async function loadIndex(options) {
	...
	const installedVersion = packages.core?.metadata.version ?? packages.wrappers.find(({ metadata }) => metadata.version)?.metadata.version;
	if (options.localIndex) return {
		envelope: parseEnvelope(await readJsonFile(path.resolve(options.localIndex)), options.localIndex),
		pathPrefix: "",
		knownVersions: [],
		source: "local-index",
	...
	try {
		...
		if (!(error instanceof SearchError) || error.status !== "version_unavailable") throw error;
		const fallback = await loadInstalledFallback(projectDir, packages);
		const explicitVersionMatchesInstalled = !options.version || Boolean(installedVersion && versionsEqual(options.version, installedVersion));
		if (fallback && explicitVersionMatchesInstalled) return fallback;
		throw error;
	}
}
function componentNameMatches(document, componentName) {
	...
	if (/^Ix[A-Z]/.test(rawQuery)) return aliases.some((alias) => typeof alias === "string" && alias.toLowerCase() === rawQuery.toLowerCase());
	...
	].filter((value) => typeof value === "string").some((value) => normalizeName(value) === query);
}
function filterMatches(document, options, effectiveKind) {
	if (document.kind !== effectiveKind) return false;
	if (options.framework && document.kind !== "component") return document.framework === options.framework;
	return true;
}
function matchingFigmaDocuments(documents, figmaId, options, effectiveKind) {
	...
	return documents.filter((document) => filterMatches(document, options, effectiveKind) && document.kind === "component" && Array.isArray(document.figmaMainComponentIds) && document.figmaMainComponentIds.some((id) => normalizeFigmaId(id).toLowerCase() === normalized));
}
function matchingComponentNameDocuments(documents, componentName, options, effectiveKind) {
	return documents.filter((document) => filterMatches(document, options, effectiveKind) && componentNameMatches(document, componentName));
}
function resultItem(result, pathPrefix, knownVersions) {
	const item = {
		...
		kind: result.kind,
		name: result.name,
		path: normalizePath(result.detailPath ?? result.path, pathPrefix, knownVersions),
		score: result.score
	};
	for (const field of [
		"tag",
		...
		"figmaMainComponentIds",
		"aliases"
	]) if (result[field] !== void 0) item[field] = result[field];
	if (Array.isArray(result.reactExamples)) item.reactExamples = result.reactExamples.map((example) => ({
		...example,
...
async function search(options, loaded) {
	...
	const addCandidate = (document, score, priority) => {
		...
		if (!current) candidates.set(document.id, {
			document,
			score,
			priority
		});
	...
	for (const figmaId of options.figmaIds) {
		...
		const matches = matchingFigmaDocuments(documents, figmaId, options, effectiveKind);
		if (matches.length === 0) {
			unmatched.push(diagnostic("figma-id", normalizeFigmaId(figmaId), "figma_main_id_unregistered", "No registered component has this Figma main component ID."));
			continue;
		}
		matchedInput = true;
	...
	const results = [...candidates.values()].sort((left, right) => right.priority - left.priority || right.score - left.score || left.document.id.localeCompare(right.document.id)).slice(0, options.limit).map(({ document, score }) => resultItem({
		...document,
		score
	}, loaded.pathPrefix, loaded.knownVersions));
	let status = "no_match";
	if (results.length > 0 || matchedInput) status = "ok";
	else if (unmatched.some(({ status: inputStatus }) => inputStatus === "figma_mapping_unavailable")) status = "figma_mapping_unavailable";
	else if (unmatched.some(({ status: inputStatus }) => inputStatus === "figma_main_id_unregistered")) status = "figma_main_id_unregistered";
...
function failureEnvelope(error) {
	return {
		status: error instanceof SearchError && error.status ? error.status : "version_unavailable",
