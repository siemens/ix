...
function parseArgs(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (name === "help") {
			console.log("Search versioned Siemens iX registry documentation. Use --local-index for offline searches.");
			process.exit(0);
		}
	...
	if (options.kind && ![
		...
		"example",
		"block"
	].includes(options.kind)) usageError(`Invalid kind '${options.kind}'`);
	if (options.framework && ![
		...
		"angular-standalone",
		"vue"
	].includes(options.framework)) usageError(`Invalid framework '${options.framework}'`);
	return options;
}
...
function exactMatchPriority(result, query) {
	...
	const names = [result.name, result.tag].filter((value) => typeof value === "string").map((value) => value.toLowerCase());
	if (names.includes(rawQuery)) return 3;
	if (names.some((value) => value.replace(/^ix-/, "") === rawQuery.replace(/^ix-/, ""))) return 2;
	if (result.figmaMainComponentIds?.some((id) => normalizeFigmaId(id.toLowerCase()) === normalizedQuery)) return 1;
	return 0;
}
function parseEnvelope(value, source) {
	...
	].every((field) => Object.prototype.hasOwnProperty.call(value.payload, field))) throw new Error(`Invalid documentation search index '${source}': expected schemaVersion 1 and a self-describing MiniSearch payload`);
	for (const [field, boost] of Object.entries(value.searchOptions.boost)) if (typeof boost !== "number" || !Number.isFinite(boost) || boost <= 0) throw new Error(`Invalid documentation search index '${source}': boost for '${field}' must be a positive number`);
	return value;
}
...
async function loadIndex(options) {
	...
	const registryResponse = await fetch(`${options.registryUrl.replace(/\/+$/, "")}/registry.json`);
	if (!registryResponse.ok) throw new Error(`Failed to fetch registry: ${registryResponse.status}`);
	const registry = await registryResponse.json();
	const latest = registry["dist-tags"]?.latest;
	const requestedVersion = options.version;
	const candidate = (requestedVersion && registry["dist-tags"]?.[requestedVersion]) ?? requestedVersion ?? latest;
	const versions = registry.versions ?? {};
	const version = candidate && Object.hasOwn(versions, candidate) ? candidate : candidate?.startsWith("v") && Object.hasOwn(versions, candidate.slice(1)) ? candidate.slice(1) : candidate && Object.hasOwn(versions, `v${candidate}`) ? `v${candidate}` : null;
	...
	if (typeof indexPath !== "string") throw new Error(`Registry version '${version}' does not define documentationSearchIndex`);
	const indexResponse = await fetch(`${options.registryUrl.replace(/\/+$/, "")}/${indexPath}`);
	if (!indexResponse.ok) throw new Error(`Failed to fetch documentation search index: ${indexResponse.status}`);
	return {
...
function main() {
	...
	return loadIndex(options).then(({ envelope, pathPrefix, knownVersions }) => {
		...
		const query = options.figmaId ? normalizeFigmaId(options.query) : options.query;
		const results = miniSearch.search(query, {
			...envelope.searchOptions,
			filter: (result) => (!options.kind || result.kind === options.kind) && (!options.framework || result.framework === options.framework)
		}).sort((left, right) => exactMatchPriority(right, query) - exactMatchPriority(left, query) || right.score - left.score).slice(0, options.limit).map((result) => {
			const item = {
				id: result.id,
				kind: result.kind,
				name: result.name,
