import { h } from "./global-Ba1Wm6ph.js";
...
import { s as shouldSuppressInternalValidation } from "./validation-VVt5EFy0-D-NZ3wqB.js";
...
function applyPaddingEnd(inputElement, width, options) {
  ...
  if (options.slotEnd) {
    inputElement.style.paddingRight = `calc(${padding} + ${options.additionalPaddingRight ?? "0rem"})`;
  } else {
...
function onInputFocus(comp, currentValue) {
  comp.initialValue = currentValue;
}
function onInputBlurWithChange(comp, input, currentValue) {
...
function onEnterKeyChangeEmit(event, comp, currentValue) {
...
function TextareaElement(props) {
  return h("textarea", { id: props.id, readOnly: props.readonly, disabled: props.disabled, maxLength: props.maxLength, minLength: props.minLength, cols: props.textareaCols, rows: props.textareaRows, ref: props.textAreaRef, class: {
    "is-invalid": props.isInvalid
  }, required: props.required, value: props.value, placeholder: props.placeholder, onFocus: () => props.onFocus?.(), onInput: (inputEvent) => {
...
function InputElement(props) {
  ...
  }, onPaste: (event) => props.onPaste?.(event), onFocus: () => props.onFocus?.(), onInput: (inputEvent) => {
    const target = inputEvent.target;
    props.updateFormInternalValue(target.value);
    props.valueChange(target.value);
...
export {
  ...
  SlotStart as a,
  addDisposableChangesAndVisibilityObservers as b,
  adjustPaddingForStartAndEnd as c,
  checkAllowedKeys as d,
  checkInternalValidity as e,
  createPickerValidityStateTracker as f,
  emitPickerValidityState as g,
  getAriaAttributesForInput as h,
  handleSubmitOnEnterKeydown as i,
  onInputBlurWithChange as j,
  onInputFocus as k,
  mapValidationResult as m,
  onEnterKeyChangeEmit as o
};
