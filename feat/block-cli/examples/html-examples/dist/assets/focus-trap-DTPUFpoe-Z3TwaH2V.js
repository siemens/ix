...
import { q as queryElements, e as focusableQueryString } from "./focus-utilities-B8qsiZ4M-BHg4MFsh.js";
...
const addFocusTrap = async (element, options) => {
  ...
  const onKeydown = (event) => {
    ...
    if (keyboardEvent.key === "Tab") {
      ...
      const focusableElements = queryElements(focusTrapRoot, `${focusableQueryString}, [${TRAP_FOCUS_INCLUDE_ATTRIBUTE}]`, useBoth);
      for (let index = 0; index < focusableElements.length; index++) {
        ...
        const includedFocusableElements = queryElements(useBoth || options?.trapFocusInShadowDom === true && includeElement.shadowRoot ? includeElement.shadowRoot ?? includeElement : includeElement, focusableQueryString, useBoth).filter((element2) => element2 !== includeElement);
