...
function requireConsts() {
  ...
  consts.morphPointsSymbol = morphPointsSymbol;
  consts.noop = noop;
  ...
  consts.transformsExecRgx = transformsExecRgx;
...
function requireGlobals() {
  ...
  const devTools = consts2.isBrowser && consts2.win.AnimeJSDevTools;
...
function requireTransforms() {
  ...
  const parseInlineTransforms = (target2, propName, animationInlineStyles) => {
    ...
    if (inlineTransforms) {
      ...
      let t;
      while (t = consts2.transformsExecRgx.exec(inlineTransforms)) {
    ...
    return inlineTransforms && !helpers2.isUnd(inlinedStylesPropertyValue) ? inlinedStylesPropertyValue : helpers2.stringStartsWith(propName, "scale") ? "1" : helpers2.stringStartsWith(propName, "rotate") || helpers2.stringStartsWith(propName, "skew") ? "0deg" : "0px";
  };
...
function requireValues() {
  ...
  const getFunctionValue = (value, target2, index, total, store) => {
    let func;
    if (helpers2.isFnc(value)) {
      func = () => {
        const computed = (
          /** @type {Function} */
          value(target2, index, total)
        );
        return !isNaN(+computed) ? +computed : computed || 0;
      };
    } else if (helpers2.isStr(value) && helpers2.stringStartsWith(value, consts2.cssVarPrefix)) {
      func = () => {
        ...
        if ((!computed || computed.trim() === consts2.emptyString) && fallbackValue) {
  ...
  const getOriginalAnimatableValue = (target2, propName, tweenType, animationInlineStyles) => {
    ...
    return type === consts2.tweenTypes.OBJECT ? target2[propName] || 0 : type === consts2.tweenTypes.ATTRIBUTE ? (
      /** @type {DOMTarget} */
      target2.getAttribute(propName)
    ) : type === consts2.tweenTypes.TRANSFORM ? transforms2.parseInlineTransforms(
      /** @type {DOMTarget} */
      target2,
      propName,
...
function requireRender() {
  ...
  const render$1 = (tickable, time2, muteCallbacks, internalRender, tickMode) => {
    ...
    if (forcedTick || tickMode === consts2.tickModes.AUTO && (time2 >= tickableDelay && time2 <= tickableEndTime || // Normal render
        ...
        while (tween) {
          ...
          if ((forcedRender || (tweenCurrentTime !== tweenChangeDuration || absoluteTime <= tweenAbsEndTime + (tweenNextRep ? tweenNextRep._delay : 0)) && (tweenCurrentTime !== 0 || absoluteTime >= tween._absoluteStartTime)) && (!tweenHasComposition || !tween._isOverridden && (!tween._isOverlapped || absoluteTime <= tweenAbsEndTime) && (!tweenNextRep || (tweenNextRep._isOverridden || absoluteTime <= tweenNextRep._absoluteStartTime)) && (!tweenPrevRep || (tweenPrevRep._isOverridden || absoluteTime >= tweenPrevRep._absoluteStartTime + tweenPrevRep._changeDuration + tween._delay)))) {
            ...
            if (tweenIsNumber) {
              ...
              const r = helpers2.round(helpers2.clamp(
                /** @type {Number} */
                tweenModifier(helpers2.lerp(fn[0], tn[0], tweenProgress)),
                0,
                255
              ), 0);
              const g = helpers2.round(helpers2.clamp(
                /** @type {Number} */
                tweenModifier(helpers2.lerp(fn[1], tn[1], tweenProgress)),
                0,
                255
              ), 0);
              const b = helpers2.round(helpers2.clamp(
                /** @type {Number} */
                tweenModifier(helpers2.lerp(fn[2], tn[2], tweenProgress)),
                0,
                255
              ), 0);
              const a = helpers2.clamp(
                /** @type {Number} */
                tweenModifier(helpers2.round(helpers2.lerp(fn[3], tn[3], tweenProgress), tweenPrecision)),
                0,
                1
              );
              value = `rgba(${r},${g},${b},${a})`;
              ...
              for (let j = 0, l = tween._toNumbers.length; j < l; j++) {
                const n = (
                  /** @type {Number} */
                  tweenModifier(helpers2.round(helpers2.lerp(tween._fromNumbers[j], tween._toNumbers[j], tweenProgress), tweenPrecision))
                );
          ...
          if (tweenTransformsNeedUpdate && tween._renderTransforms) {
            ...
            for (let key in tweenTargetTransformsProperties) {
              str += `${consts2.transformsFragmentStrings[key]}${tweenTargetTransformsProperties[key]}) `;
            }
...
function requireStyles() {
  ...
  const cleanInlineStyles = (renderable) => {
    if (renderable._hasChildren) {
      helpers2.forEachChildren(renderable, cleanInlineStyles, true);
    } else {
      const animation2 = (
        /** @type {JSAnimation} */
        renderable
      ...
      helpers2.forEachChildren(animation2, (tween) => {
        ...
        if (tweenTarget[consts2.isDomSymbol]) {
          const targetStyle = (
            /** @type {DOMTarget} */
            tweenTarget.style
          );
          ...
          if (tween._tweenType === consts2.tweenTypes.TRANSFORM) {
            ...
            if (tween._renderTransforms) {
              if (!Object.keys(cachedTransforms).length) {
                ...
                for (let key in cachedTransforms) {
                  str += consts2.transformsFragmentStrings[key] + cachedTransforms[key] + ") ";
                }
...
function requireClock() {
  ...
  class Clock {
    ...
    requestTick(time2) {
      ...
      if (time2 < scheduledTime) return consts2.tickModes.NONE;
...
function requireEngine() {
  ...
  class Engine extends clock2.Clock {
    ...
    set speed(playbackRate) {
      this._speed = playbackRate * globals2.globals.timeScale;
...
function requireTimer() {
  ...
  class Timer extends clock2.Clock {
    ...
    constructor(parameters = {}, parent = null, parentPosition = 0) {
      ...
      if (globals2.devTools) {
        const isInfinite = timerIterationCount === Infinity;
        const registered = globals2.devTools.register(this, parameters, isInfinite);
        if (registered && isInfinite) {
          const minIterations = alternate ? 2 : 1;
          const iterations = parent ? globals2.devTools.maxNestedInfiniteLoops : globals2.devTools.maxInfiniteLoops;
    ...
    resume() {
      ...
      if (this.duration <= consts2.minValue && !this._hasChildren) {
        ...
        if (!this._running) {
          helpers2.addChild(engine2.engine, this);
...
function requireAnimation() {
  ...
  class JSAnimation extends timer2.Timer {
     ...
     */
    constructor(targets$1, parameters, parent, parentPosition, fastSet = false, index = 0, length = 0) {
      super(
        ...
        for (let p in params) {
          if (helpers2.isKey(p)) {
            ...
            for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
              ...
              const computedToValue = values2.getFunctionValue(key.to, target2, ti, tl, toFunctionStore);
              let tweenToValue;
              ...
              const tweenFromValue = values2.getFunctionValue(key.from, target2, ti, tl);
              const easeToParse = key.ease || tEasing;
              const easeFunctionResult = values2.getFunctionValue(easeToParse, target2, ti, tl);
              ...
              ) : values2.getFunctionValue(values2.setValue(key.duration, l > 1 ? values2.getFunctionValue(tDuration, target2, ti, tl) / l : tDuration), target2, ti, tl);
              const tweenDelay = values2.getFunctionValue(values2.setValue(key.delay, !tweenIndex ? tDelay : 0), target2, ti, tl);
              const computedComposition = values2.getFunctionValue(values2.setValue(key.composition, tComposition), target2, ti, tl);
              ...
              if (tweenComposition !== consts2.compositionTypes.none) {
                if (!siblings) siblings = composition2.getTweenSiblings(target2, propName);
              ...
              if (isFromToValue) {
                values2.decomposeRawValue(isFromToArray ? values2.getFunctionValue(tweenToValue[0], target2, ti, tl, fromFunctionStore) : tweenFromValue, fromTargetObject);
                values2.decomposeRawValue(isFromToArray ? values2.getFunctionValue(tweenToValue[1], target2, ti, tl, toFunctionStore) : tweenToValue, toTargetObject);
                ...
                if (hasFromvalue) {
                  ...
                  if (prevTween) {
                    ...
                    values2.decomposeRawValue(parent && prevSibling && prevSibling.parent.parent === parent ? prevSibling._value : (
              ...
              const tween = {
                ...
                _fromNumbers: helpers2.cloneArray(fromTargetObject.d),
                _toNumbers: helpers2.cloneArray(toTargetObject.d),
                _strings: helpers2.cloneArray(toTargetObject.s),
                _fromNumber: fromTargetObject.n,
                _toNumber: toTargetObject.n,
                _numbers: helpers2.cloneArray(fromTargetObject.d),
    ...
    revert() {
      super.revert();
      return styles2.cleanInlineStyles(this);
    }
    /**
  ...
  const animate = (targets3, parameters) => new JSAnimation(targets3, parameters, null, 0, false).init();
...
function requireTimeline() {
  ...
  function addTlChild(childParams, tl, timePosition, targets3, index, length) {
  ...
  class Timeline extends timer2.Timer {
          ...
          if (helpers2.isFnc(a3)) {
            ...
            parsedTargetsArray.forEach((target2) => {
              ...
              addTlChild(
                ...
                this,
                position2.parseTimelinePosition(this, staggeredPosition(target2, i, parsedLength, this)),
                target2,
                i,
                parsedLength
              );
            ...
            addTlChild(
              childParams,
              this,
              position2.parseTimelinePosition(this, a3),
              /** @type {TargetsParam} */
              a1
            );
          }
    ...
    sync(synced, position3) {
      ...
      return this.add(synced, { currentTime: [0, duration], duration, delay: 0, ease: "linear", playbackEase: "linear" }, position3);
    }
    /**
     * @param  {TargetsParam} targets
     * @param  {AnimationParams} parameters
...
function requireWaapi() {
  ...
  const parseIndividualTweenValue = ($el, propName, from, to, i, targetsLength) => {
    let tweenValue = "0";
    const computedTo = !helpers2.isUnd(to) ? normalizeTweenValue(propName, to, $el, i, targetsLength) : getComputedStyle($el)[propName];
    if (!helpers2.isUnd(from)) {
      const computedFrom = normalizeTweenValue(propName, from, $el, i, targetsLength);
      tweenValue = [computedFrom, computedTo];
    } else {
      tweenValue = helpers2.isArr(to) ? to.map((v) => normalizeTweenValue(propName, v, $el, i, targetsLength)) : computedTo;
    }
    return tweenValue;
  };
  class WAAPIAnimation {
    ...
    constructor(targets$1, params) {
      ...
      parsedTargets.forEach(($el, i) => {
        ...
        const easeFunctionResult = values2.getFunctionValue(easeToParse, $el, i, targetsLength);
        const keyEasing = helpers2.isFnc(easeFunctionResult) || helpers2.isStr(easeFunctionResult) ? easeFunctionResult : easeToParse;
        const spring2 = (
          /** @type {Spring} */
        ...
        ) : values2.getFunctionValue(values2.setValue(params.duration, globals2.globals.defaults.duration), $el, i, targetsLength)) * timeScale;
        const delay = values2.getFunctionValue(values2.setValue(params.delay, globals2.globals.defaults.delay), $el, i, targetsLength) * timeScale;
        const composite = (
        ...
        for (let name in params) {
          ...
          if (helpers2.isObj(propertyValue)) {
            ...
            ) : values2.getFunctionValue(values2.setValue(tweenOptions.duration, duration), $el, i, targetsLength)) * timeScale;
            tweenParams.delay = values2.getFunctionValue(values2.setValue(tweenOptions.delay, delay), $el, i, targetsLength) * timeScale;
            ...
            parsedPropertyValue = helpers2.isArr(propertyValue) ? propertyValue.map((v) => normalizeTweenValue(name, v, $el, i, targetsLength)) : normalizeTweenValue(
              name,
              /** @type {any} */
              propertyValue,
              $el,
...
function requireLayout() {
  ...
  const updateNodeTimingParams = (node, params) => {
    const easeFunctionResult = values2.getFunctionValue(params.ease, node.$el, node.index, node.total);
    ...
    node.duration = hasSpring ? (
      /** @type {Spring} */
      keyEasing.settlingDuration
    ) : values2.getFunctionValue(params.duration, node.$el, node.index, node.total);
    node.delay = values2.getFunctionValue(params.delay, node.$el, node.index, node.total);
  };
    ...
    animate(params = {}) {
      ...
      newState.forEachRootNode((node) => {
        ...
        for (let prop in nodeProperties) {
          nodeProperties[prop] = values2.getFunctionValue(nodeProperties[prop], $el, node.index, node.total);
          oldStateNodeProperties[prop] = values2.getFunctionValue(oldStateNodeProperties[prop], $el, oldStateNode.index, oldStateNode.total);
        }
      ...
      if (transformedLength) {
        for (let i = 0; i < transformedLength; i++) {
          ...
          if (animatedSwap.includes($el)) {
            node.ease = values2.getFunctionValue(swapAtParams.ease, $el, node.index, node.total);
            node.duration = values2.getFunctionValue(swapAtParams.duration, $el, node.index, node.total);
          }
        }
        this.transformAnimation = waapi2.waapi.animate(transformed, {
          ...
          transform: ($el) => {
            ...
            const node = newState.getNode($el);
            return [oldValue, values2.getFunctionValue(swapAtProps.transform, $el, node.index, node.total), newValue];
          },
          autoplay: false,
          // persist: true,
          ...timingParams
...
function requireStagger() {
  ...
  const stagger$1 = (val, params = {}) => {
    ...
    return (target2, i, t, tl) => {
      const [registeredTarget] = targets2.registerTargets(target2);
      const total = helpers2.isUnd(customTotal) ? t : customTotal;
      ...
      const staggerIndex = helpers2.isNum(customIndex) || helpers2.isStr(customIndex) && helpers2.isNum(+customIndex) ? +customIndex : i;
      ...
      if (!values$1.length) {
        for (let index = 0; index < total; index++) {
          if (!grid) {
            ...
            const fromY = !fromCenter ? helpers2.floor(fromIndex / grid[0]) : (grid[1] - 1) / 2;
            const toX = index % grid[0];
            const toY = helpers2.floor(index / grid[0]);
            const distanceX = fromX - toX;
            const distanceY = fromY - toY;
            let value = helpers2.sqrt(distanceX * distanceX + distanceY * distanceY);
            if (axis === "x") value = -distanceX;
            if (axis === "y") value = -distanceY;
            values$1.push(value);
          }
          maxValue = helpers2.max(...values$1);
        }
        if (staggerEase) values$1 = values$1.map((val3) => staggerEase(val3 / maxValue) * maxValue);
        if (reversed) values$1 = values$1.map((val3) => axis ? val3 < 0 ? val3 * -1 : -val3 : helpers2.abs(maxValue - val3));
        if (fromRandom) values$1 = random2.shuffle(values$1);
      }
      const spacing = isRange ? (val2 - val1) / maxValue : val1;
      const offset = tl ? position2.parseTimelinePosition(tl, helpers2.isUnd(params.start) ? tl.iterationDuration : start) : (
        /** @type {Number} */
        start
      );
      let output = offset + (spacing * helpers2.round(values$1[staggerIndex], 2) || 0);
...
function requireMorphto() {
  ...
  const morphTo = (path2, precision = 0.33) => ($path1) => {
    ...
    const previousPoints = $path1[consts2.morphPointsSymbol];
    ...
    $path1[consts2.morphPointsSymbol] = v2;
    return [v1, v2];
  };
...
function requireSplit() {
  ...
  class TextSplitter {
     ...
     */
    addEffect(effect) {
      if (!helpers2.isFnc(effect)) return console.warn("Effect must return a function.");
