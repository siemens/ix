...
function requireValues() {
  ...
  const composeColorValue = (tween, progress, precision) => {
    ...
    const r = helpers2.round(helpers2.clamp(
      /** @type {Number} */
      mod(helpers2.lerp(fn[0], tn[0], progress)),
      0,
      255
    ), 0);
    const g = helpers2.round(helpers2.clamp(
      /** @type {Number} */
      mod(helpers2.lerp(fn[1], tn[1], progress)),
      0,
      255
    ), 0);
    const b = helpers2.round(helpers2.clamp(
      /** @type {Number} */
      mod(helpers2.lerp(fn[2], tn[2], progress)),
      0,
      255
    ), 0);
    const a = helpers2.clamp(
      /** @type {Number} */
      mod(helpers2.round(helpers2.lerp(fn[3], tn[3], progress), precision)),
      0,
      1
    );
  ...
  const composeComplexValue = (tween, progress, precision) => {
    ...
    const hasComposition = tween._composition !== consts2.compositionTypes.none;
    let v = ts[0];
        ...
        while (tween) {
          ...
          if ((forcedRender || (tweenCurrentTime !== tweenChangeDuration || absoluteTime <= tweenAbsEndTime + (tweenNextRep ? tweenNextRep._delay : 0)) && (tweenCurrentTime !== 0 || absoluteTime >= tween._absoluteStartTime)) && (!tweenHasComposition || !tween._isOverridden && (!tween._isOverlapped || absoluteTime <= tweenAbsEndTime) && (!tweenNextRep || (tweenNextRep._isOverridden || absoluteTime <= tweenNextRep._absoluteStartTime)) && (!tweenPrevRep || (tweenPrevRep._isOverridden || absoluteTime >= tweenPrevRep._absoluteStartTime + tweenPrevRep._changeDuration + tween._delay)))) {
            ...
            if (tweenIsNumber) {
              ...
              value = values2.composeColorValue(tween, tweenProgress, tweenPrecision);
  ...
  const revertValues = (renderable, inlineStylesOnly = false) => {
    if (renderable._hasChildren) {
      ...
      helpers2.forEachChildren(animation2, (tween) => {
        ...
        if (tweenType === consts2.tweenTypes.OBJECT) {
        ...
        for (let p in params) {
          if (helpers2.isKey(p)) {
            ...
            for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
              ...
              const tweenFromValue = values2.getFunctionValue(key.from, target2, ti, tl, null, prevSiblingTween);
              ...
              if (vt === consts2.valueTypes.COMPLEX) {
                ...
                tween._value = values2.composeColorValue(tween, 1, -1);
          ...
          for (let index = 0; index < total; index++) {
            if (!grid) {
              ...
              if (fromArr) {
                ...
                fromY = helpers2.floor(fromIndex / grid[0]);
              }
