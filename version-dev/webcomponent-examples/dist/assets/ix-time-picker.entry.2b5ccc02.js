import { r as registerInstance, c as createEvent, h, H as Host } from "./index.ed9c6cc1.js";
...
const TimePicker = class {
  ...
  watchTimePropHandler(newValue) {
    this._time = luxon.DateTime.fromFormat(newValue, this.format);
  ...
  componentWillLoad() {
    ...
    this._timeRef = this.format.includes("a") ? luxon.DateTime.fromFormat(this.time, this.format).toFormat("a") : void 0;
    this.formatTime();
  }
  formatTime() {
  ...
  timeUpdate(unit, value) {
    let maxValue = luxon.DateTime.now().endOf("day").get(unit);
  ...
  render() {
    ...
    return h(Host, null, h("ix-date-time-card", { standaloneAppearance: this.standaloneAppearance, corners: this.corners }, h("div", { class: "header", slot: "header" }, h("ix-typography", { variant: "default-title" }, this.textTime || "Time")), h("div", { class: "clock" }, timepickerInformation.map((descriptor, index) => h("div", { class: "flex" }, h("div", { class: { columns: true, hidden: descriptor.hidden } }, h("ix-icon-button", { size: "16", onClick: () => this._time = this._time.plus({
      [descriptor.unit]: 1
