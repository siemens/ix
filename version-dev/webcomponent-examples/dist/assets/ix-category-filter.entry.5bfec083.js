...
const CategoryFilter = class {
  ...
  render() {
    var _a;
    return h(Host, null, h("form", { ref: (el) => this.formElement = el }, h("div", { "read-only": this.readonly, class: {
      "input-container": true,
      disabled: this.disabled,
      focus: this.hasFocus,
      readonly: this.readonly,
      "no-icon": this.hideIcon
    } }, h("ix-icon", { color: this.getIconColor(), class: { "d-none": this.hideIcon }, name: this.icon, size: "16" }), h("div", { class: "token-container" }, h("ul", { class: "list-unstyled" }, this.filterTokens.map((value, index) => h("li", { key: value.toString(), class: {
      animate__animated: true,
      animate__fadein: true
    } }, h("ix-filter-chip", { disabled: this.disabled, readonly: this.readonly, onCloseClick: () => this.removeToken(index) }, this.getFilterChipLabel(value)))), this.categories === void 0 ? "" : h("li", { class: {
    ...
    }, disabled: this.disabled, readonly: this.readonly, ref: (el) => this.textInput = el, type: "text", placeholder: this.placeholder }))), this.getResetButton())), this.disabled || this.readonly ? "" : h("ix-dropdown", { closeBehavior: "outside", offset: { mainAxis: 2 }, trigger: this.textInput, triggerEvent: ["click", "focus"], header: this.getDropdownHeader() }, this.renderDropdownContent()));
  }
  get hostElement() {
