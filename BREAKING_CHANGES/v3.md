...
To load individual icons you can also use the `addIconToCache` function during runtime:

```js
import { addIconToCache } from @siemens/ix-icons;
import { iconStar } from @siemens/ix-icons/icons;

addIconToCache('myStarIcon', iconStar)
```

```html
<ix-icon name="my-star-icon"></ix-icon>
````
